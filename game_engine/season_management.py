...
class SeasonManager:
    ...
    def create_season(
        ...
        try:
            ...
            for country_id in participating_countries:
                domestic_competitions = self._schedule_domestic_competitions(season_id, country_id)
                competitions_created += len(domestic_competitions)
            
            continental_competitions = self._schedule_continental_competitions(season_id, participating_countries)
    ...
    def _schedule_domestic_competitions(self, season_id: str, country_id: str) -> List[Competition]:
        ...
        for tier in range(1, self.season_config["TOTAL_DIVISIONS"] + 1):
            clubs = self.firestore_helper.get_clubs_by_country_and_tier(country_id, tier)
    ...
    def _create_national_cup(self, season_id: str, country_id: str) -> Optional[Competition]:
        ...
        participants = [
            CompetitionParticipant(
                club_id=club["id"],
                seeded=(club.get("divisionTier", 19) <= 5)  # Top 5 divisions are seeded
            ) 
        ...
        competition = Competition(
            ...
            country_id=country_id,
            format=CompetitionFormatDetails(
                type=CompetitionFormat.KNOCKOUT,
                teams_count=len(participants)
            ),
            participants=participants,
            prizes=prizes,
    ...
    def _schedule_continental_competitions(self, season_id: str, participating_countries: List[str]) -> List[Competition]:
        """Schedule continental competitions"""
        competitions = []
        
        champions_league = self._create_continental_champions_league(season_id, participating_countries)
        if champions_league:
            competitions.append(champions_league)
        
        professional_cup = self._create_continental_professional_cup(season_id, participating_countries)
        if professional_cup:
            competitions.append(professional_cup)
        
        amateur_championship = self._create_continental_amateur_championship(season_id, participating_countries)
        if amateur_championship:
            competitions.append(amateur_championship)
        
        return competitions
    
    def _create_continental_champions_league(self, season_id: str, participating_countries: List[str]) -> Optional[Competition]:
        ...
        competition_id = str(uuid.uuid4())
        participants = [CompetitionParticipant(club_id=club["id"], seeded=True) for club in elite_clubs]
    ...
    def _create_continental_professional_cup(self, season_id: str, participating_countries: List[str]) -> Optional[Competition]:
        ...
        competition_id = str(uuid.uuid4())
        participants = [CompetitionParticipant(club_id=club["id"]) for club in professional_clubs]
    ...
    def _create_continental_amateur_championship(self, season_id: str, participating_countries: List[str]) -> Optional[Competition]:
        ...
        competition_id = str(uuid.uuid4())
        participants = [CompetitionParticipant(club_id=club["id"]) for club in amateur_clubs]
