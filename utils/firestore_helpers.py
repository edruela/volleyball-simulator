...
class FirestoreHelper:
    ...
    def create_club(self, club_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            club = Club(
                ...
                division_tier=club_data.get('divisionTier', random.randint(10, 19)),  # Start in amateur
    ...
    def generate_initial_squad(self, club_id: str, country_id: str, division_tier: int = 10) -> List[str]:
        """Generate initial squad of players for a new club"""
        try:
            positions = [
                'OH', 'OH', 'OH',  # 3 Outside Hitters
                'MB', 'MB',        # 2 Middle Blockers
                'OPP', 'OPP',      # 2 Opposite Hitters
                'S', 'S',          # 2 Setters
                'L',               # 1 Libero
                'DS', 'DS'         # 2 Defensive Specialists
            ]
    ...
    def get_league_standings(self, country_id: str, division_tier: int) -> List[Dict[str, Any]]:
        """Get league standings for a specific division"""
        try:
            clubs_ref = self.db.collection('clubs').where('countryId', '==', country_id).where('divisionTier', '==', division_tier)
            standings = []
            ...
            standings.sort(key=lambda x: (x['points'], x['setsWon'] - x['setsLost']), reverse=True)
    ...
    def create_sample_data(self):
        ...
        try:
            countries = [
                {'id': 'volcania', 'name': 'Volcania', 'modifiers': {'blockTiming': 15, 'strength': 10, 'agility': -5}},
                {'id': 'coastalia', 'name': 'Coastalia', 'modifiers': {'agility': 10, 'serveAccuracy': 10, 'strength': -5}},
                {'id': 'forestland', 'name': 'Forestland', 'modifiers': {'spikePower': 5, 'blockTiming': 5, 'passingAccuracy': 5}}
            ]
